...
3. **El registro encontró tres problemas reales en esta máquina** el primer día: hooks fuera del
   subdirectorio actual, las tres entradas de hooks en el formato de una instalación anterior, y
   la entrada MCP de Codex puesta a mano. Ninguno era visible antes.
4. **Ampliación fuera del plan, justificada:** el check de hooks registrados distingue la
   instalación anterior (formato `args`, scripts en otra ruta) y la reporta `warn`. Cabe en
   REQ-002 y son cinco líneas.
5. **Falso positivo propio, corregido:** ese mismo detalle decía primero «que Claude Code no
   ejecuta … están muertas», copiando un comentario de `install.py`. **Es falso** — `args` es el
   exec form del schema y se ejecuta; verificado dos veces, la última viendo disparar
   `suggest_lint_summary.py` en vivo. Es la lección más útil del change: un registro que existe
   para eliminar falsos positivos casi publica uno, por creerle a un comentario del repo en vez
   de a la evidencia. Se corrigieron el detalle y el comentario de `install.py` que lo originó.
6. **Cambio de comportamiento a documentar:** el backend caído ahora cuenta como aviso (exit 1),
   antes no. Lo exige el escenario de aceptación de la spec y está en el CHANGELOG.
7. **Matiz del escenario «HOME limpio»:** con un HOME totalmente vacío todo sale `[ -- ]`, no
