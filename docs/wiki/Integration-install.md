...
| `--enable-read-hook` | registra **y enciende** los dos hooks de lectura: `PreToolUse`/`Read` y `PreToolUse`/`Bash|PowerShell`. Van juntos porque son una regla sola sobre dos caminos —cerrar `Read` y dejar `cat informe.md` abierto solo muda la conducta de sitio—. Registra además el mismo hook para las tools de lectura de otros MCP (`mcp__*__read_*`, `mcp__*__get_file_contents`), que solo dejan el evento (`camino: mcp`) y nunca bloquean. Antes solo registraba: el script exigía además `LD_HOOK_READ_ENABLED=1` y la bandera no encendía nada. El **bloqueo** viene aparte y apagado (`LD_HOOK_READ_BLOQUEAR`, ver Configuration) |
